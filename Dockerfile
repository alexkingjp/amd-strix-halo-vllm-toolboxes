...
RUN cd /tmp && \
  dnf -y install msgpack-devel libzstd-devel && \
...
# 9. Install Custom RCCL (gfx1151) - SKIPPED (binary not in repo)
# The TheRock SDK ships with a working RCCL for single-GPU.
# Uncomment below if you have the custom librccl.so.1.gz:
